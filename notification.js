...
(function() {
  ...
  // ✅ REAL VAPID public key (replaces the fake placeholder)
  const PUBLIC_VAPID_KEY = "BJVaExyNyglKk9Gnlpppsg6bfNLegaIjmc-G2Ll9wdjTd2NSy1wF56hZZRRF-PwTEzXn4e1vOXEfyQCA1oFGBXE";

  let rtClient = null;
  let toastDebounce = new Set(); // Prevents duplicate toasts
  ...
  function ensureSupabaseClient(callback) {
    // ✅ Reuse the shared client if it exists (avoids GoTrueClient warning + duplicate connections)
    if (window.supabaseClient) {
      rtClient = window.supabaseClient;
      callback(rtClient);
      return;
    }
    ...
    if (window.supabase && typeof window.supabase.createClient === 'function') {
      if (!rtClient) {
        ...
        window.supabaseClient = rtClient;
      }
      callback(rtClient);
      return;
    }
  ...
  function showInAppToast(title, body, type = 'info') {
    // ✅ Debounce: prevent duplicate toasts for the same title within 3 seconds
    const key = `${title}|${body}`;
    if (toastDebounce.has(key)) return;
    toastDebounce.add(key);
    setTimeout(() => toastDebounce.delete(key), 3000);
  ...
  window.triggerPushNotification = async function(title, body, url = '/', channelType = null) {
    ...
    if (Notification.permission === 'granted') {
      const options = {
        ...
        badge: '/logo.png',
        vibrate: [200, 100, 200],
        tag: 'tt-alert-' + Date.now() + '-' + Math.random().toString(36).slice(2, 8), // ✅ unique tag
        renotify: true,
  ...
  window.initTirzaTrimRealtime = function() {
    ensureSupabaseClient((sb) => {
      ...
      const channel = sb.channel('public:universal_notification_sync');

      channel
        .on('postgres_changes', { event: '*', schema: 'public', table: 'orders' }, payload => {
          const o = payload.new || payload.old || {};
          console.log('⚡ [Global Sync Order Event]', payload.eventType, o);
          ...
          if (payload.eventType === 'UPDATE' && payload.old) {
            ...
            if (oldStatus !== newStatus && isRep && o.rep_code === currentRep?.territory_code) {
              let icon = '📦';
              let type = 'order';
              let msg = '';
              ...
              if (newStatus === 'Delivered') {
                icon = '✅';
                msg = `Order ${o.order_id || ''} for ${patientText} has been delivered`;
              } else if (newStatus === 'Dispatched' || newStatus === 'Out for Delivery') {
                icon = '🚚';
                msg = `Order ${o.order_id || ''} for ${patientText} is on the way`;
              } else if (newStatus === 'Cancelled') {
                icon = '❌';
                type = 'alert';
                msg = `Order ${o.order_id || ''} for ${patientText} was cancelled`;
              } else if (newStatus === 'Order Processing' || newStatus === 'Prescription Received') {
                icon = '📝';
                msg = `Order ${o.order_id || ''} is being processed`;
              }

              if (msg) {
                window.triggerPushNotification(`${icon} Order Update`, msg, '/team.html', type);
              }
            }
          }
        })
        ...
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'notifications' }, payload => {
          const n = payload.new || {};
          // Only notify if this notification belongs to the currently logged-in user
          const mySap = currentRep?.sap_id || currentManager?.manager_sap_id || currentHOS?.sap_id;
          if (mySap && String(n.user_id) === String(mySap) && !n.is_read) {
            console.log('🔔 [Notification Row]', n.title);
            const typeMap = { order: 'order', feedback: 'feedback', broadcast: 'broadcast', system: 'info' };
            window.triggerPushNotification(n.title || 'Update', n.message || '', n.link || '/', typeMap[n.type] || 'info');
          }
        })
        .subscribe((status) => {
          if (status === 'SUBSCRIBED') {
            console.log('✅ Universal realtime sync connected');
          } else if (status === 'CHANNEL_ERROR') {
            console.warn('⚠️ Universal realtime sync error, will retry...');
          }
        });
    });
  };
